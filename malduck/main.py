...
def extract(ctx, paths, base, analysis, modules):
    ...
    if not extract_manager.extractors:
        click.echo("[!] No extractor modules found under '{}'!".format(
            modules), err=True)
        ctx.abort()
    ...
    for path in paths:
        if os.path.isdir(path):
            files = filter(os.path.isfile, map(
                lambda f: os.path.join(path, f), os.listdir(path)))
        elif os.path.isfile(path):
            files = [path]
        else:
            files = []
            click.echo(
                "[!] Symbolic links are not supported, {} ignored.".format(path), err=True)
